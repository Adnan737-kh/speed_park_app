...
import 'package:speed_park_app/ui/lobby_controller_views/sales_report/sales_report.dart';

import '../../../../../widgets/appBar.dart';
import '../../super_user_views/create_location/view.dart';
...
class GenerateSalesReport extends StatelessWidget {
  GenerateSalesReport({Key? key}) : super(key: key);

  final homeScreen = Get.put(CreateReportLogic());
  final logic = Get.put(GenerateReportFromFilterLogicDay());
  final state = Get.find<GenerateReportFromFilterLogicDay>().state;

  @override
  Widget build(BuildContext context) {
    Get.lazyPut(() => GenerateReportFromFilterLogicDay());
    return SafeArea(
      child: Scaffold(
                    ...
                    onPressed: () async {
                      ...
                      if (logic.filteredCarList.isEmpty) {
                        ...
                        Get.snackbar('Result', 'Result found');
                              await Get.to(() => SalesReport(dataSet:logic.filteredCarList,));
                      }
                    },
