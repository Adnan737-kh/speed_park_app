...
class ListViewPage extends StatelessWidget {
  ...
  ListViewPage({super.key, required this.dataSet});

  Future<Uint8List> buildPrintablePage(List<CarHistoryModel> dataSet) async {
  ...
  String buildHtmlContent(List<CarHistoryModel> dataSet) {
    ...
    String dataRows = dataSet.map((car) {
      double amountIncludeTax =
          double.tryParse(car.amountIncludeTax.toString()) ?? 0.0;
    ...
    return SafeArea(
      child: Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () async {
            Printing.layoutPdf(
              onLayout: (PdfPageFormat format) async => await buildPrintablePage(dataSet),
            );
          },
